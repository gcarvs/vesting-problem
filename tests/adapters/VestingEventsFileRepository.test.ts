...
describe("Test VestingEventsFileRepository", () => {
    ...
    describe("test processVestingEventsFile function", () => {
        ...
        test("test for all valid rows", () => {
            ...
            const result = repository.processVestingEventsFile(mockVestingEventsFile);

            expect(spyConvertCsvRow).toHaveBeenCalledTimes(mockVestingEventsFileRows.length);
            expect(result).toMatchObject(mockVestingEvents);
        });
        ...
        test("test for some invalid rows", () => {
            ...
            expect(spyConvertCsvRow).toHaveBeenCalledTimes(mockVestingEventsFileRows.length);
            expect(result).toMatchObject([mockVestingEvents[1]]);
        });
    });
    ...
    describe("test getSharesVestedByTargetDate function", () => {
        ...
        test("test for a valid file name", async () => {
            ...
            spyProcessFile.mockReturnValueOnce(mockVestingEvents);

            const result = await repository.getVestingEventsHistory();
            expect(spyProcessFile).toHaveBeenCalledWith(fileContent);
            expect(result).toMatchObject(mockVestingEvents);
        });
